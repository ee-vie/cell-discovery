tool
extends Path

export var distance_between_planks = 1.0
#	set(value):
#		distance_between_planks = value
#		is_dirty = true

var is_dirty = false

# Called when the node enters the scene tree for the first time.
func _ready():
	_update_multimesh()
	pass # Replace with function body.


func _process(_delta):
	if is_dirty:
		_update_multimesh()

		is_dirty = false

func _update_multimesh():
	var path_length: float = curve.get_baked_length()
	var count = floor(path_length / distance_between_planks)
	
	var mm: MultiMesh = $Bases.multimesh
	mm.instance_count = count
	var offset = distance_between_planks/2.0
	
	for i in range(0, count):
		var curve_distance = offset + distance_between_planks * i
		var position = curve.interpolate_baked(curve_distance, true)
		
		var basis = Basis()
		
		var up = curve.interpolate_baked_up_vector(curve_distance, true)
		var forward = position.direction_to(curve.interpolate_baked(curve_distance + 0.1, true))
		
		basis.y = up
		basis.x = forward.cross(up).normalized()
		basis.z = -forward 
		
		var transform = Transform(basis, position)
		mm.set_instance_transform(i, transform)


func _on_DNAPath_curve_changed():
	is_dirty = true

